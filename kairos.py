...
def get_beaufort(kmh):
    ...
    else: return 8 # Για Γήλοφο, πάνω από 8 είναι θύελλα!

def get_weather():
    try:
        # 1. Λήψη δεδομένων (Προσθήκη daily για ανατολή/δύση)
        ...
        # 2. Υπολογισμός Ανέμου με Μποφόρ
        wind_cardinal = get_direction(wind_deg)
        bft = get_beaufort(wind_spd)
        wind_info = f"{wind_deg}° {wind_cardinal} ({bft} Μπφ)"
        
        # 3. Δυναμικός έλεγχος Νύχτας από το API
        sunset_str = daily['sunset'][0].split("T")[1] # Παίρνει το "18:09"
        sunrise_str = daily['sunrise'][0].split("T")[1]
        ...
        if precip > 0:
            ...
            if clouds <= 20: 
                # Το ΞΑΣΤΕΡΙΑ.ΑΙΘΡΙΟΣ πλέον ακολουθεί τον ήλιο
        ...
        weather_data = {
            ...
            "wind_speed": wind_spd,
            "wind_dir": wind_deg,
            "wind_text": wind_info, # Εδώ μπήκε η νέα πληροφορία
            "rain": precip,
            "clouds": clouds,
            "status": weather_type,
            "time": time_now_str,
        ...
        print(f"[{time_now_str}] {weather_type} | Άνεμος: {wind_info}")
